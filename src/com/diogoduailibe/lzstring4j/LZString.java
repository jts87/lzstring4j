package webponent.export.util.compress;
...
public class LZString {
    ...
    public static String compress(String uncompressed) {
        int bitsPerChar = 16;
        return compress(uncompressed, bitsPerChar, null);
    }

    private static String compress(String uncompressed, int bitsPerChar, String type) {
                    ...
                    if (((int)context_w.charAt(0)) < 256) {
                        for (int i = 0; i < context_numBits; i++) {
                            ...
                            if (context_data_position == bitsPerChar - 1) {
                                ...
                                context_data_string += getCharFromInt(context_data_val, type);
                                context_data_val = 0;
                            } else {
    ...
    private static char getCharFromInt (int context_data_val, String type) {
        if ("encodedURI".equals(type)) {
            return keyStrUriSafe.charAt(context_data_val);

        } else {
            return (char) context_data_val;
        }
    }
    ...
    public static String decompress(String compressed) {
        int resetValue = 32768;
        return decompress(compressed, resetValue, null);
    }

    private static String decompress(String compressed, int resetValue, String type) {
        if (compressed == null)
            return "";
        if (compressed == "")
            return null;
        ...
        data.val = getNextValue(compressed, 0, type);/*(int) compressed.charAt(0);*/
        ...
        while (power != Double.valueOf(maxpower).intValue()) {
            ...
            if (data.position == 0) {
                data.position = resetValue;
                data.val = getNextValue(data.string, data.index++, type);
            }
    ...
    private static int getNextValue(String compressed, int index, String type) {
        char charIdx = compressed.charAt(index);

        if ("encodedURI".equals(type)) {
            return keyStrUriSafe.indexOf(charIdx);
    ...
    public static String encode64(String input) {
        ...
        for (int i = 0, max = input.length() << 1; i < max;) {
            ...
            if (left >= 3) {
                ...
                i++;
                result.append(keyStrBase64.charAt((ch1 >> 2) & 0x3f));
                result.append(keyStrBase64.charAt(((ch1 << 4) + (ch2 >> 4)) & 0x3f));
                result.append(keyStrBase64.charAt(((ch2 << 2) + (ch3 >> 6)) & 0x3f));
                result.append(keyStrBase64.charAt(ch3 & 0x3f));
            } else if (left == 2) {
                int ch1 = (input.charAt(i >> 1) >> ((1 - (i & 1)) << 3)) & 0xff;
                i++;
                int ch2 = (input.charAt(i >> 1) >> ((1 - (i & 1)) << 3)) & 0xff;
                ...
                result.append(keyStrBase64.charAt((ch1 >> 2) & 0x3f));
                result.append(keyStrBase64.charAt(((ch1 << 4) + (ch2 >> 4)) & 0x3f));
                result.append(keyStrBase64.charAt(((ch2 << 2)) & 0x3f));
                result.append('=');
            } else if (left == 1) {
                int ch1 = (input.charAt(i >> 1) >> ((1 - (i & 1)) << 3)) & 0xff;
                i++;
                result.append(keyStrBase64.charAt((ch1 >> 2) & 0x3f));
                result.append(keyStrBase64.charAt(((ch1 << 4)) & 0x3f));
                result.append('=');
                result.append('=');
            }
        }
